...
export const registerUser = async(data) => {
    try{
      const response = await axoisInstance.post(`${APIS.USERS_API}/register`, data)
...
export const deleteUser = async(_id) => {
    try{
      const response = await  axoisInstance.delete(`${APIS.USERS_API}/allusers/${_id}`)
      const user = await response?.data
      if(!user){
...
export const updateRole = async(payload) => {
    let {_id}= payload;
    try{
      const response = await axoisInstance.put(`${APIS.USERS_API}/allusers/${_id}`, payload)
      const user = await response?.data
      if(!user){
...
export const userValidd = async(params) => {
    try{
        await axoisInstance.get(`${APIS.USERS_API}/forgotpassword/${params.id}/${params.token}`)
    }catch(e){
      toast.error("error", e.message)
    } 
}
